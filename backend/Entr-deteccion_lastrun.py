...
expName = 'Entr-deteccion'  # from the Builder filename that created this script
expInfo = {
    'participant': '',
    'session': '001',
}
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\Natasha\\Documents\\PsychoPy\\Entrenamiento deteccion\\Entr-deteccion_lastrun.py',
...
for thisLoop1 in loop1:
    ...
    routineForceEnded = False
    # update component parameters for each repeat
    sound1.setSound(Sonido, secs=4.0, hamming=True)
    ...
    # --- Run Routine "bloque1" ---
    while continueRoutine and routineTimer.getTime() < 14.1:
        # get current time
        t = routineTimer.getTime()
        tThisFlip = win.getFutureFlipTime(clock=routineTimer)
        tThisFlipGlobal = win.getFutureFlipTime(clock=None)
        ...
        # *text* updates
        if text.status == NOT_STARTED and tThisFlip >= 4.1-frameTolerance:
            # keep track of start time/frame for later
        ...
        # *key_resp* updates
        waitOnFlip = False
        if key_resp.status == NOT_STARTED and tThisFlip >= 4.1-frameTolerance:
            # keep track of start time/frame for later
    ...
    if routineForceEnded:
        ...
        routineTimer.addTime(-14.100000)
...
for thisLoop2 in loop2:
    ...
    routineForceEnded = False
    # update component parameters for each repeat
    sound_2.setSound(sonido, secs=9.0, hamming=True)
    ...
    # --- Run Routine "bloque2" ---
    while continueRoutine and routineTimer.getTime() < 19.1:
        # get current time
        t = routineTimer.getTime()
        tThisFlip = win.getFutureFlipTime(clock=routineTimer)
        tThisFlipGlobal = win.getFutureFlipTime(clock=None)
        ...
        if sound_2.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > sound_2.tStartRefresh + 9.0-frameTolerance:
                # keep track of stop time/frame for later
        ...
        # *text1* updates
        if text1.status == NOT_STARTED and tThisFlip >= 9.1-frameTolerance:
            # keep track of start time/frame for later
        ...
        # *key_resp_3* updates
        waitOnFlip = False
        if key_resp_3.status == NOT_STARTED and tThisFlip >= 9.1-frameTolerance:
            # keep track of start time/frame for later
    ...
    if routineForceEnded:
        ...
        routineTimer.addTime(-19.100000)
    thisExp.nextEntry()
...
for thisLoop3 in loop3:
    ...
    routineForceEnded = False
    # update component parameters for each repeat
    sound_1.setSound(sonido, secs=9.0, hamming=True)
    ...
    while continueRoutine and routineTimer.getTime() < 19.1:
        ...
        if sound_1.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > sound_1.tStartRefresh + 9.0-frameTolerance:
                # keep track of stop time/frame for later
        ...
        if image_2.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > image_2.tStartRefresh + 9.0-frameTolerance:
                # keep track of stop time/frame for later
        ...
        # *text2* updates
        if text2.status == NOT_STARTED and tThisFlip >= 9.1-frameTolerance:
            # keep track of start time/frame for later
        ...
        # *resp_2* updates
        waitOnFlip = False
        if resp_2.status == NOT_STARTED and tThisFlip >= 9.1-frameTolerance:
            # keep track of start time/frame for later
